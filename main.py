...
def generate_dataset():
    ...
    data = generator(material="oil", theta_list=[np.pi / 4, np.pi / 3, np.pi / 2, 3 * np.pi / 4],
                     single=True, add_noise=False, draw_fig=False, method="cumulate")
    ...
    experiment, draw = True, True
    # np.save(f"./data/{data.material}/{num_angle}/{symbol_single}/dataset/{symbol_line}feature{symbol_noise}.npy", data_x)
    # np.save(f"./data/{data.material}/{num_angle}/{symbol_single}/dataset/{symbol_line}target{symbol_noise}.npy", data_y)

    # data_x = np.load(
    #     f"./data/{data.material}/{num_angle}/{symbol_single}/dataset/{symbol_line}feature{symbol_noise}.npy")
    # data_y = np.load(
    #     f"./data/{data.material}/{num_angle}/{symbol_single}/dataset/{symbol_line}target{symbol_noise}.npy")
    # if data.add_noise:
    ...
    x_train = np.load(
        f"./data/{data.material}/{num_angle}/{symbol_single}/dataset/train/{symbol_line}feature{symbol_noise}.npy")
    y_train = np.load(
        f"./data/{data.material}/{num_angle}/{symbol_single}/dataset/train/{symbol_line}target{symbol_noise}.npy")
    x_test = np.load(
        f"./data/{data.material}/{num_angle}/{symbol_single}/dataset/test/{symbol_line}feature{symbol_noise}.npy")
    y_test = np.load(
        f"./data/{data.material}/{num_angle}/{symbol_single}/dataset/test/{symbol_line}target{symbol_noise}.npy")
    ...
    t = np.linspace(100, 1200, y_test.shape[1], endpoint=True) if data.single else np.linspace(300, 900,
                                                                                               y_test.shape[1],
                                                                                               endpoint=True)
    # svr = joblib.load(f"./data/oil/multi-angle/{symbol_single}/model/svr{symbol_noise}.pkl")
    sk_model = "RFR"
    if sk_model == "GRNN":
    ...
    for index in range(y_predict.shape[0]):
        performance, _, _ = utils.get_line(t, y_predict[index], y_test[index], data.single,
                                           method="fit line")  # "use maximum"
        performance_li[index] = performance
    print(f"测试集峰均值相对误差为{np.mean(performance_li) * 100:.2f}%")

    if draw:
        test_shuffle = np.random.randint(0, x_test.shape[0], 20)  # 从测试集中随机抽取5个样本出来可视化结果
        min_index = np.argmin(V)
        max_index = np.argmax(V)
        print("%.2f" % V[min_index], "%.2f" % V[max_index])
        min_max_index = [min_index, max_index]
        for i in min_max_index:  # 看测试集上前5个的预测情况
            Dg_estimate, origin_mean, predict_mean = utils.get_line(t, y_predict[i], y_test[i],
                                                                    data.single, method="fit line")
            if data.single:
            ...
            plt.scatter(t, y_predict[i], color="red", linewidth=1, label=sk_model)
            # plt.scatter(t, y_predict_svr[i], color="blue", linewidth=1, label="Tikhonov")
            plt.xlabel("diameter (nm)", fontsize=13)
            plt.ylabel("f(x)", fontsize=13)
            plt.title("Use Random Forest Regressor to Predict PSD")
